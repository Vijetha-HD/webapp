pipeline { 
    ...
    stages { 
            ...
            steps { 
                withCredentials([usernamePassword(credentialsId: 'jenkins', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) { 
                    sh ''' 
                        ...
                        docker tag insta $DOCKER_USER/insta:latest 
                        docker push $DOCKER_USER/insta:latest 
                    ''' 
                } 
            } 
            post { 
        ...
        stage("Remove Docker Image Locally") { 
            steps { 
                sh """ 
                docker rmi -f ${DOCKERHUB_USERNAME}/insta || true 
                docker rmi -f insta || true 
                """ 
            } 
            post { 
                success { 
        ...
        stage("Stop and Restart Container") { 
            steps { 
                ...
                docker run -d --name app -p 8081:8080 ${DOCKERHUB_USERNAME}/insta 
                """ 
            } 
            post { 
                success { 
